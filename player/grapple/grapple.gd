...
# Touch support: stores the global position of the last touch used to aim the grapple.
var _last_touch_position := Vector2.ZERO
# Which finger index started the current grapple, so only that finger can release it (-1 = none).
var _grapple_finger := -1
...
func _unhandled_input(event: InputEvent) -> void:
	# Grapple is driven purely by real touch events (InputEventScreenTouch). On
	# desktop, "emulate_touch_from_mouse" turns a click into a touch so testing still
	# works. We intentionally do NOT react to raw mouse-button events: with
	# "emulate_mouse_from_touch" on (needed so the pause menu is tappable), every touch
	# also spawns a fake mouse click, and reacting to it would fire the grapple even
	# when the touch was meant for an on-screen button.
	#
	# Touches that land on a HUD button are consumed by touch_controls.gd (it calls
	# set_input_as_handled), so they never reach here. Only touches on empty space do.
	# We remember which finger started the grapple so lifting a *different* finger
	# (e.g. releasing the JUMP button) doesn't cancel an active swing.
	if not (event is InputEventScreenTouch):
		return
	var touch := event as InputEventScreenTouch
	if touch.pressed:
		_last_touch_position = get_canvas_transform().affine_inverse() * touch.position
		if _state == State.IDLE and input_enabled and _can_grapple:
			_grapple_finger = touch.index
			_set_target()
			SoundController.play(shoot_grapple_sound, -8, randf_range(1.0, 1.2))
			_state = State.EXTENDING
			hook.global_position = global_position
	elif touch.index == _grapple_finger:
		_grapple_finger = -1
...
func _extend_process(delta: float) -> void:	
	...
	if hook_raycast.is_colliding():
		...
		if collider is Enemy:
			# Hit an enemy — damage it, then retract (the hook does not stick).
			(collider as Enemy).take_grapple_hit()
			hook.global_position = hook_raycast.get_collision_point()
			_state = State.RETRACTING

		elif collider is GrappleArea:
			var grapple_area := collider as GrappleArea
			# Hit a grappleable.
...
func _set_target() -> void:
	# On mobile use the recorded touch position; on desktop use the mouse position.
	var aim_pos: Vector2
	if _last_touch_position != Vector2.ZERO:
		aim_pos = _last_touch_position
	else:
		aim_pos = get_global_mouse_position()

	var grapple_area := _get_grapple_area(aim_pos)
	if grapple_area == null:
		_target = aim_pos
		_is_target_grappable = false
	else:
		_target = grapple_area.get_grapple_point(aim_pos)
