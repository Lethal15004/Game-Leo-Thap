...
func _ready() -> void:
	# So enemies can locate the player (used to trigger their attack animation).
	add_to_group("player")
...
func _physics_process(delta: float) -> void:
	if freeze_position or _dead:
		# When dead, hold still (hit-stop) while the red flash plays before respawn.
		return
...
# Called by an Enemy when the player runs into it (not a stomp). Plays the hit
# sound, flashes the player red as feedback, then respawns at the last checkpoint.
func hit_by_enemy() -> void:
	if _dead:
		return
	_dead = true
	SoundController.play(_hit_sound)
	# Freeze the player during the flash (keep the current sprite, don't swap it).
	_input_enabled = false
	_grapple.input_enabled = false
	velocity = Vector2.ZERO
	var t := create_tween()
	t.tween_property(_sprite, "modulate", Color(2.0, 0.4, 0.4), 0.06)
